...
export function getObjectDepth(obj) {
  ...
  let depth = 0;
  for (const key in obj) depth = Math.max(depth, getObjectDepth(obj[key]));
  return 1 + depth;
}
...
export function runLatencyAnalysis(jsonData, rawText) {
  ...
  <div class="p-5 rounded-xl border border-${s1Color}-200 bg-${s1Color}-50">
    ...
    <div class="bg-white p-3 rounded-lg border border-${s1Color}-100 text-xs font-mono mb-4 text-slate-700">
      ...
      &bull; Approx Tokens: <strong>${approxTokens.toLocaleString()}</strong><br/>
      &bull; Nesting Depth: <strong>${depth} level(s)</strong><br/>
      &bull; Assessed Complexity: <strong>${complexityLabel}</strong>
    </div>
  ...
  <div class="relative rounded-xl overflow-hidden">
    <div class="p-5 border border-amber-200 bg-amber-50 blur-sm pointer-events-none select-none" aria-hidden="true">
      <h3 class="font-bold text-slate-800 mb-2 flex items-center gap-2">
        <i class="fa-solid fa-graduation-cap text-amber-600"></i>
        2. Reduce "Instruction Overhead"
      </h3>
      <p class="text-slate-700 mb-2 text-sm">General models require massive context windows to understand specific schemas. Fine-tuning eliminates prompt boilerplate.</p>
      <div class="bg-white p-3 rounded border border-slate-100 text-xs font-mono mb-2">
        <strong>Check:</strong> Structure Type: Array of Objects | Item Count: 12
      </div>
      <p class="text-amber-800 font-medium text-sm">💡 <strong>Actionable Fix: Fine-tune this schema.</strong> Fine-tuning a smaller model on this schema allows it to operate on "muscle memory," significantly improving TTFT (Time-to-First-Token).</p>
    </div>
    <div class="absolute inset-0 flex items-center justify-center bg-white/50">
      <span class="bg-slate-800 text-white text-xs font-bold px-4 py-2 rounded-full tracking-widest uppercase shadow-lg flex items-center gap-2">
        <i class="fa-solid fa-lock text-slate-300"></i> Coming Soon
      </span>
    </div>
  </div>`;
  ...
  html += `
  <div class="relative rounded-xl overflow-hidden">
    <div class="p-5 border border-purple-200 bg-purple-50 blur-sm pointer-events-none select-none" aria-hidden="true">
      <h3 class="font-bold text-slate-800 mb-2 flex items-center gap-2">
      ...
      <p class="text-slate-600 text-sm">&minus; Due to depth and complexity, normal thinking budgets should be maintained to prevent structural hallucinations.</p>
    </div>
    <div class="absolute inset-0 flex items-center justify-center bg-white/50">
      <span class="bg-slate-800 text-white text-xs font-bold px-4 py-2 rounded-full tracking-widest uppercase shadow-lg flex items-center gap-2">
        <i class="fa-solid fa-lock text-slate-300"></i> Coming Soon
